...
try:
    ...
    def _on_collision(data):
        ...
        actor = get_actor_display_name(data.other_actor)
        print("Collision with", actor)
        record = data.normal_impulse
        intensity = math.sqrt(record.x ** 2 + record.y **2 + record.z **2)
        print("Intesity of collision", intensity)
        dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
        time.sleep(5)
